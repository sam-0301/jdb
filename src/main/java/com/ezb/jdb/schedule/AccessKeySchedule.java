...
public class AccessKeySchedule {
    ...
    public void expireToken() {
        int size = accessKeyServiceImpl.expireDay(Constants.ACCKEY_TOKEN,1);
        log.info("expireToken " + size + " accessKey success");
    }

    @Scheduled(cron = "0 0/10 * * * ?")
    public void expireInvatate() {
        int size = accessKeyServiceImpl.expireMinute(Constants.ACCKEY_INVITATE, 30);
        log.info("expireInvatate " + size + " accessKey success");
    }

    @Scheduled(cron = "0 0 0/1 * * ?")
    public void expireActivity() {
        int size = accessKeyServiceImpl.expireHour(Constants.ACCKEY_ACTIVITY, 1);
        log.info("expireActivity " + size + " accessKey success");
    }

    @Scheduled(cron = "0 0 0/1 * * ?")
    public void expireAtvcmt() {
        int size = accessKeyServiceImpl.expireHour(Constants.ACCKEY_ATVCMT, 1);
        log.info("expireAtvcmt " + size + " accessKey success");
    }

    @Scheduled(cron = "0 0 0/1 * * ?")
    public void expireTopiccmt() {
        int size = accessKeyServiceImpl.expireHour(Constants.ACCKEY_TOPICCMT, 1);
        log.info("expireTopiccmt " + size + " accessKey success");
    }

    @Scheduled(cron = "0 0 0/1 * * ?")
    public void expireTopic() {
        int size = accessKeyServiceImpl.expireHour(Constants.ACCKEY_TOPIC, 1);
