...
public class TopicCmtServiceImpl implements ITopicCmtService {
    ...
    public String likeTopicCmt(String phone, Integer id) {
        User user = userDao.queryByPhone(phone);
        if (null != user) {
            String accessKey = DateTime.now().toString(Constants.HOUR_FMT) + phone + "likeTopicCmt" + id;
            if (!accessKeyDao.exist(accessKey, Constants.ACCKEY_TOPICCMT)) {
                accessKeyDao.add(accessKey,Constants.ACCKEY_TOPICCMT);
