...
public class ActivityServiceImpl implements IActivityService {
    ...
    public String queryActivity(PageResult<Activity> pageResult, String phone, String queryWords) {
        User user = userDao.queryByPhone(phone);
        if (null == user) {
            return ResponseState.INVALID_PHONE;
        }
        pageResult = activityDao.queryActivity(pageResult, queryWords);
    ...
    public String viewActivity(String phone, Integer id) {
        ...
        String accessKey = JdbMd5Util.Md5(DateTime.now().toString(Constants.HOUR_FMT) + phone + "viewActivity" + id);

        if (!accessKeyDao.exist(accessKey,Constants.ACCKEY_ACTIVITY)) {
            activity.setPv(activity.getPv() + 1);
            activityDao.update(activity);
            accessKeyDao.add(accessKey,Constants.ACCKEY_ACTIVITY);
        }
