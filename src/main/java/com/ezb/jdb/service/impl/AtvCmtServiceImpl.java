...
public class AtvCmtServiceImpl implements IAtvCmtService {
    ...
    public String likeAtvCmt(String phone, Integer id) {
        User user = userDao.queryByPhone(phone);
        if (null != user) {
            String accessKey = DateTime.now().toString(Constants.HOUR_FMT) + phone + "likeAtvCmt" + id;
            if (!accessKeyDao.exist(accessKey, Constants.ACCKEY_ATVCMT)) {
                accessKeyDao.add(accessKey,Constants.ACCKEY_ATVCMT);
