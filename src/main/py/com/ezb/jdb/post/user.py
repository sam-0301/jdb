...
def login():
    """登录"""
    url = "http://localhost:8000/jdb/mobile/user/login"
    params = {"username": "1111111", "password": "pass1"}
...
if __name__ == '__main__':
    token = login()
    url = "http://localhost:8000/jdb/mobile/image/upload"
    register_openers()
    params = {"pic1": open("aa.png", "rb"),
              "pic2": open("bb.png", "rb")}
    datagen, headers = multipart_encode(params)
    request = urllib2.Request(url, datagen, headers)
    request.add_header("x-access-token", token)
